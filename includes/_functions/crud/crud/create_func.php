...
if(isset($_GET["insert"])){
	...
	if(uriParam('tmp') == encryptStringArray('templates/logsheet/index.php')){
		$warning = cmsDB();
		$nama_field_jam =  listGetAt($field_name, 4);
		$jam = $_POST[$nama_field_jam."_timehour"].":".$_POST[$nama_field_jam."_timeminute"].":00";
		$check_sql = "select * from ref_logsheet where 
						group_location_id='".postParam(listGetAt($field_name, 1))."' and 
						unit_id='".postParam(listGetAt($field_name, 2))."' and 
						date='".date2sql(postParamSimple(listGetAt($field_name, 3)))."' and 
						HOUR(jam)='".$jam."' and 
						is_deleted=0";

		$warning->query($check_sql);

		if($warning->recordCount()){
	    	echo "<script>$('.loader-wrapper').hide();</script>";

			echo "<script>alert('Gagal input! Data logsheet dalam jam tersebut telah dimasukkan');location='".$_SERVER["SCRIPT_NAME"]."?tmp=".uriParam("tmp")."&refresh=".md5("mdYHis")."';</script>";
			die();
		}
	}
	...
	for($field=1;$field<=listLen($field_name);$field++){
		...
		}elseif($ftype=="multiselect" || $ftype=="moreselect" || $ftype=="multi-checkbox"){
		...
		}elseif($ftype=="date-now" || $ftype=="date-now-readonly"){
