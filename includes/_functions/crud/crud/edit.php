    		...
    		<div class="col-md-12">
				<div class="card">
					...
					if(uriParam('tmp') == encryptStringArray('templates/logsheet/index.php')){
						echo _add_production('edit', $edit->row('group_location_id'), $edit->row('date'));
					}
					?>
					...
					<form action="<?$_SERVER["SCRIPT_NAME"]?>?tmp=<?=uriParam("tmp")?>&<?=$primary_id?>=<?=uriParam($primary_id)?>&edit=yes&update=yes&refresh=<?=md5("mdYHis")?>" method="post" name="updateform" enctype="multipart/form-data" id="updateform" class="form-horizontal">
						...
						<?if($field_type_ori=="text" || $field_type_ori=="telephone"){
							edit_text($field_name_ori, $edit->row($field_name_ori), $field_size, $required, $field_row);
							?>
						...
						<?}elseif($field_type_ori=="date" || $field_type_ori=="date-now"){?>
							<input name="<?=$field_name_ori?>" class="datepicker-here form-control digits" size="10" type="text" value="<?=datesql2date($edit->row($field_name_ori))?>"  <?=$required?> /> 
						...
						<?}elseif($field_type_ori=="select"){
								edit_select($field_name_ori, $field_size, $required, $edit->row($field_name_ori));
						...
						<?}elseif($field_type_ori=="multi-checkbox"){
				                ...
				                <?while($select_table->next()){?>
							 		<div class="checkbox checkbox-primary">
							        	<input id="<?=$select_table->row('id')?>" type="checkbox" value="<?=$select_table->row("id")?>" name="<?=$field_name_ori?>[]" data-original-title="" title="" <? if(listFind($edit->row($field_name_ori),$select_table->row("id"))){ echo " checked";}?>>
				...
				<?
				if($child_enable == 'yes'){
					_child($primary_id, $child_enable,$child_name,$child_main_query,$nama_table_child,$primary_id_child,$field_name_child,$field_type_child,$field_child_require,$record_show_child, "newformchild", "insert_child", "update_child", "delete_child");
				}

				if($child_enable_2 == 'yes'){
					_child($primary_id, $child_enable_2,$child_name_2,$child_main_query_2,$nama_table_child_2,$primary_id_child_2,$field_name_child_2,$field_type_child_2,$field_child_require_2,$record_show_child_2, "newformchild_2", "insert_child_2", "update_child_2", "delete_child_2");
				}

				if($child_enable_3 == 'yes'){
					_child($primary_id, $child_enable_3,$child_name_3,$child_main_query_3,$nama_table_child_3,$primary_id_child_3,$field_name_child_3,$field_type_child_3,$field_child_require_3,$record_show_child_3, "newformchild_3", "insert_child_3", "update_child_3", "delete_child_3");
				}
