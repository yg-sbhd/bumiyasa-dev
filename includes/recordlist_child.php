<?
function _child($primary_id, $child_enable,$child_name,$child_main_query,$nama_table_child,$primary_id_child,$field_name_child,$field_type_child,$field_child_require,$record_show_child,$newformchild,$insert_child,$update_child,$delete_child){
	global $_POST,$_GET;
...
if(isset($_GET[$delete_child])){
	...
	// echo "<script>location='".$_SERVER["SCRIPT_NAME"]. "?tmp=".uriParam("tmp")."&edit=yes&".$primary_id."=".uriParam($primary_id)."&refresh=".md5("mdYHis")."#child';</script>";die();
	echo "<script>location='".$_SERVER["SCRIPT_NAME"]. "?tmp=".uriParam("tmp")."&edit=yes&".$_primary_id."=".uriParam($_primary_id)."&refresh=".md5("mdYHis")."#child';</script>";
	die();
	...
	<div class="card-body">
		...
		<div class="row px-3" style="border-bottom: 1px solid #eee;">
			...
			for($a=1;$a<=listLen($field_type_child, '#');$a++){
				...
				$field_alias_name=listGetAt($field_detail,4,"|"); //Ambil Field Type
			...
			<div class="col-<?=$field_row?> p-1"><?=$field_alias_name?></div>
			<?}?>
		</div>
		<div class="row px-3">
			<?
			...
			while ($show_detail->next()) {
				...
				<div class="col-md-12">
					<div class="row">
						<div class="col-1 p-1">
							<a href="#" data-toggle="modal" data-target="#<?=$primary_id_child?>-<?=$show_detail->row($primary_id_child)?>" data-original-title="" title="">
								<?=$no;?>
							</a>
						</div>
						...
						for($field=1;$field<=listLen($record_show_child,"~");$field++){
							...
							$field_row = listGetAt($field_type_child,$field,"#");
							$field_row = listGetAt(listGetAt($field_row,3,"|"),2,"^");

		            	?>
							<div class="col-<?=$field_row?> p-1" align="<?=$row_align?>">
							...
							if(listFind($row_query,"select"," ")){
								...
								echo $val_fname;	
							}elseif($row_query == "date"){
								echo datesql2date($show_detail->row($row_field));
							}elseif($row_query == "number"){
								echo number_format($show_detail->row($row_field));
							}elseif($row_query == "decimal"){
								echo number_format($show_detail->row($row_field),2);
							}else{
				...
				<div class="modal fade" id="<?=$primary_id_child?>-<?=$show_detail->row($primary_id_child)?>" tabindex="-1" role="dialog" aria-labelledby="<?=$primary_id_child?>-<?=$show_detail->row($primary_id_child)?>" style="display: none;" aria-hidden="true">
						...
						<div class="modal-content">
	 						<form action="<?$_SERVER["SCRIPT_NAME"]?>?tmp=<?=uriParam("tmp")?>&edit=yes&<?=$_primary_id?>=<?=uriParam($_primary_id)?>&<?=$update_child?>=yes&refresh=<?=md5("mdYHis")?>#child" method="post" name="updatechildform2" enctype="multipart/form-data" id="updateform" class="form-horizontal">
									...
									<div class="row px-3">
										<?
							          	for($field=1;$field<=listLen($record_show_child,"~");$field++){
											...
											$row_sort = listGetAt($row_detail,4,"|");
							             ?>
											<div class="col p-1" align="<?=$row_align?>"><?=$row_aliasName?></div>
										<?}?>
									</div>
									<div class="row px-3">
										...
										?>
											<div class="col p-1">
												...
												if($field_type_ori == "select"){
													...
													edit_date($field_name_ori, $show_detail->row($field_name_ori), $field_size, $required, $field_row);
												}else{
								...
								<div class="modal-footer">
									...
									<button type="button" class="btn btn-secondary btn-sm" onclick="if(confirm('Anda Yakin Hapus Data Ini?')){ location='<?$_SERVER["SCRIPT_NAME"]?>?tmp=<?=uriParam("tmp")?>&edit=yes&<?=$_primary_id?>=<?=uriParam($_primary_id)?>&<?=$delete_child?>=yes&<?=$primary_id_child?>=<?=$show_detail->row($primary_id_child)?>&ref=<?=md5(date("mdyHis"))?>#child';} " data-target="#child">Delete</button>
		...
		<form action="<?=$_SERVER["SCRIPT_NAME"]?>?tmp=<?=uriParam("tmp")?>&edit=yes&<?=$_primary_id?>=<?=uriParam($_primary_id)?>&<?=$insert_child?>=yes&refresh=<?=md5("mdYHis")?>" method="post" name="<?=$newformchild?>" enctype="multipart/form-data" id="<?=$newformchild?>" class="form-horizontal">
			...
			<div class="row px-3">
				<div class="col" align="center">
					<button type="button" class="btn btn-success btn-sm" onclick="_cekChildValid_<?=$newformchild?>(document.<?=$newformchild?>);">
...
if(listLen($field_child_require)){
	?>
		<script type="text/javascript">
			function _cekChildValid_<?=$newformchild?>(frm){
				<?
				for($req=1;$req<=listLen($field_child_require);$req++){
					...
					?>
					if(frm.<?=trim($nmfield)?>.value=='' || frm.<?=trim($nmfield)?>.value==' '){ 
