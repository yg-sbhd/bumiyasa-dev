      ...
      <div class="table-scrollable">
         <table class="table table-striped table-bordered table-hover">
            <thead>
               <tr>
                  ...
                  <td style="vertical-align : middle;text-align:center;" rowspan="4" colspan="7">
                     <h2><b><?=$show_unit->row('group_name')?></b></h2></td>
                  ...
                  <td colspan="20"><?=getDayIndonesia(Date2SQLnoTime($date))?></td>
            ...
            <tbody>
               <?php
               $detail = cmsDB();
               $q = "select * from ref_logsheet where group_location_id=$site and unit_id=$unit_id and date='".Date2SQLnoTime($date)."'";
               $detail->query($q);
               ...
               foreach($array as $value) {?>
                  <tr>
                     ...
                     <?
                     $qry = "select * from ref_logsheet where group_location_id=$site and unit_id=$unit_id and date='".Date2SQLnoTime($date)."' and hour(jam)='".$value."'";
                     $detail->query($qry);
                     ?>
                     <?
               ...
               <tr>
                 ...
                 <?
                  $max = cmsDB();
                  $qry_max = "select * from master_logsheet where group_location_id=$site and unit_id=$unit_id and min_max='Max'";
                  $max->query($qry_max);
                  if ($max->recordCount()) {
                     while ($max->next()) {?>
               ...
               <tr>
                 ...
                 <?
                  $min = cmsDB();
                  $qry_min = "select * from master_logsheet where group_location_id=$site and unit_id=$unit_id and min_max='Min'";
                  $min->query($qry_min);
                  if ($min->recordCount()) {
                     while ($min->next()) {?>
